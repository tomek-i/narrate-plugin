...
async function loadChromium() {
  try {
    ...
    throw new Error(
      "Playwright is not installed. Run `narrate setup` once to install it and the Chromium browser, then retry."
    );
  }
}
var PlaywrightRecorder = class {
...
async function smoothScrollTo(page, targetY, overMs) {
  await page.evaluate(
    async ({ targetY: targetY2, overMs: overMs2 }) => {
      const start = window.scrollY;
      const dist = targetY2 - start;
      if (overMs2 <= 0) {
        window.scrollTo(0, targetY2);
        return;
      }
      const steps = Math.max(1, Math.round(overMs2 / 40));
      for (let i = 1; i <= steps; i++) {
        window.scrollTo(0, start + dist * (i / steps));
        await new Promise((r) => setTimeout(r, overMs2 / steps));
      }
    },
    { targetY, overMs }
  );
...
function parseRate(mime, fallback = 24e3) {
  const m = /rate=(\d+)/.exec(mime ?? "");
  return m ? Number(m[1]) : fallback;
}

// src/tts/gemini.ts
var DEFAULT_MODEL2 = "gemini-2.5-flash-preview-tts";
var GeminiProvider = class {
  constructor(key, voice = "Kore", model = DEFAULT_MODEL2) {
    this.key = key;
    this.voice = voice;
    this.model = model;
  }
...
program2.name("narrate").description("Generate a narrated walkthrough video of a website.").version("0.21.0");
